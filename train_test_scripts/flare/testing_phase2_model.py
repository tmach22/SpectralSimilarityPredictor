...
def evaluate_model(args):
    ...
    print(f"[*] Loading Stage 2 Siamese Weights from: {args.stage2_ckpt}")
    ...
    with torch.no_grad():
        for b_A, b_B, b_meta, b_massA, b_massB, labels, brics_A, brics_B in tqdm(test_loader, desc="Evaluating"):
            ...
            preds = (probs > 0.5).astype(float) # Cast to float to match your requested structure (1.0 / 0.0)
    ...
    # Because shuffle=False, our predictions map 1:1 with the original pairs dataframe
    results_df = test_dataset.pairs_df.copy()
    
    # Ensure standard label naming
    if 'label' in results_df.columns:
        results_df = results_df.rename(columns={'label': 'true_label'})
    
    # Append our neural network predictions
    results_df['prob_similarity'] = all_probs
    results_df['predicted_label'] = all_preds
    results_df['mass_diff'] = all_mass_diffs # Used temporarily for terminal prints
    
    # Fallback in case 'cosine_similarity' wasn't natively in the feather file
    ...
    # Reorder columns to exactly match the requested format
    ...
    csv_path = os.path.join(args.output_dir, "siamese_test_predictions.csv")
    final_output_df.to_csv(csv_path, index=False)
    print(f"\n[+] Detailed predictions saved to {csv_path}")
    print("[+] Output structure successfully verified against requested format.")

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--stage1_ckpt", type=str, required=True, help="Path to FLARE motif weights")
    parser.add_argument("--stage2_ckpt", type=str, required=True, help="Path to best Siamese Sinkhorn weights")
