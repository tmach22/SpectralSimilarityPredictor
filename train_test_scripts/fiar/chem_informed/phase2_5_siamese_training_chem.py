...
try:
    ...
    # Import the newly updated Phase 3 Partial Thaw Architecture
    from phase2_5_siamese_model_chem import Phase3_Partial_SiameseNetwork 
...
def train_phase3(args):
    ...
    # =================================================================
    # 2-TIER OPTIMIZER (Anchoring the Bottleneck)
    # =================================================================
    ...
    optimizer = AdamW([
        {'params': edge_head_params, 'lr': args.lr_edge_head},    # 5e-6 (Thawed Backbone)
        {'params': bottleneck_params, 'lr': args.lr_bottleneck}   # 1e-5 (Stabilized Anchoring)
    ], weight_decay=1e-3)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (batch_A, batch_B, targets) in enumerate(train_bar):
            ...
            if torch.isnan(loss):
                print("\n[!] FATAL: NaN loss detected. Halting.")
                wandb.finish()
                sys.exit(1)
                
            loss.backward()
            
            all_trainable = edge_head_params + list(bottleneck_params)
