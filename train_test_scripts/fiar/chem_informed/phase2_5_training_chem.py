...
sys.path.insert(0, os.path.join(cwd, 'model', 'fiar', 'chem_informed'))
...
try:
    from phase2_5_dataloader_chem import PairedSiameseDataset, siamese_collate_fn
    from phase2_5_model_chem import Phase2_5_SiameseNetwork 
...
def train_phase2_5(args):
    ...
    # =================================================================
    # [THE FIX] DIFFERENTIAL PHASE 2.5 OPTIMIZER
    # =================================================================
    attn_params = list(model.context_attn.parameters())
    
    adapter_params = (
        list(model.electronic_film.parameters()) + 
        list(model.mass_encoder.parameters()) + 
        list(model.post_mass_layernorm.parameters()) + 
        list(model.final_affine.parameters())
    )
    
    optimizer = AdamW([
        {'params': attn_params, 'lr': args.lr_attn},         # 1e-5 (Protected)
        {'params': adapter_params, 'lr': args.lr_adapters}   # 1e-3 (Rapid mapping)
    ], weight_decay=1e-3)
    ...
    print("\n[*] Commencing Phase 2.5 Differential Training Loop...")
    for epoch in range(1, args.epochs + 1):
        
        # The Temperature Annealing Schedule
        if epoch == 1:
            current_tau = 0.1
        elif epoch == 2:
            current_tau = 0.03
        ...
        for batch_idx, (batch_A, batch_B, targets) in enumerate(train_bar):
            ...
            all_trainable = attn_params + list(adapter_params)
            ...
            train_bar.set_postfix({'Huber': f"{loss.item():.4f}", 'GradNorm': f"{total_norm.item():.2f}"})
        ...
        wandb.log({
            ...
            "val/pred_std": p_std,
            "lr_attn": optimizer.param_groups[0]['lr'],
            "lr_adapters": optimizer.param_groups[1]['lr']
        })
...
if __name__ == '__main__':
    ...
    # [UPDATED] Differential Learning Rates
    parser.add_argument("--lr_attn", type=float, default=1e-5) 
    parser.add_argument("--lr_adapters", type=float, default=1e-3) 
