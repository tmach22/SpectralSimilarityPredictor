...
try:
    # Ensure this dataloader incorporates the RDKit thermodynamics injection!
...
def train_linear_probe(args):
    ...
    # --- SAFETY CHECK FOR STRICT=FALSE ---
    if os.path.exists(args.phase3_ckpt):
        print(f"[*] Loading pretrained Phase 2.5 Partial Thaw weights: {args.phase3_ckpt}")
        missing_keys, unexpected_keys = model.load_state_dict(torch.load(args.phase3_ckpt, map_location=device), strict=False)
        
        print("\n--- Weight Loading Diagnostics ---")
        print(f"Missing Keys (Should ONLY be binary_head): \n{missing_keys}")
        print(f"Unexpected Keys (Should be empty): \n{unexpected_keys}")
        print("----------------------------------\n")
        
        assert all("binary_head" in k for k in missing_keys), "\n[!] FATAL: A core layer failed to load! Check your namespaces."
    
    # --- EXPLICIT MANIFOLD FREEZE ---
    print("[*] Executing Topological Lockdown (Freezing Backbone and Adapters)...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (batch_A, batch_B, targets_sim, targets_label) in enumerate(train_bar):
            ...
            # Force both outputs and targets to flat 1D tensors
