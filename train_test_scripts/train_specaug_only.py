...
def train_specaugment_only(args):
    ...
    print(f"--- Starting Phase 5b: Full SpecAugment Fine-Tuning (With Mass Gating) ---")
    ...
    try:
        ...
        print(f"WARNING: Slight mismatch in loading weights (likely due to new Mass Gating head dimension): {e}")
        print("This is expected. The Similarity Head will be re-initialized/re-trained.")

    # 5. Unfreezing Strategy (Head + Last 2 Layers)
    print("\n--- Unfreezing Head + Last 2 Graph Layers ---")
    for param in model.parameters(): param.requires_grad = False
    
    # Head (Must be unfrozen to learn Mass Gating weights)
    ...
    for epoch in range(args.epochs):
        ...
        # UPDATED LOOP: Unpack mass_diffs
        for batch_A, batch_B, batch_meta, mass_diffs, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
            for k in batch_A: batch_A[k] = batch_A[k].to(device)
            for k in batch_B: batch_B[k] = batch_B[k].to(device)
            batch_meta = batch_meta.to(device)
            mass_diffs = mass_diffs.to(device) # <--- Move to Device
            ...
            # UPDATED FORWARD: Pass mass_diffs
            logits = model(aug_A, aug_B, batch_meta, mass_diffs).view(-1)
        ...
        with torch.no_grad():
            for batch_A, batch_B, batch_meta, mass_diffs, labels in tqdm(val_loader, desc="Validating"):
                for k in batch_A: batch_A[k] = batch_A[k].to(device)
                for k in batch_B: batch_B[k] = batch_B[k].to(device)
                batch_meta = batch_meta.to(device)
                mass_diffs = mass_diffs.to(device) # <--- Move to Device
        ...
        if val_f1 > best_val_f1:
            ...
            save_path = os.path.join(args.output_dir, "best_specaugment_only_model_mass_gated.pth")
