...
try:
    ...
    # Ensure we use the correct reverted loader
    from binary_data_loader import BinaryClassificationDataset, binary_collate_fn
    
    # Simple config merge helper if updated_train is missing
...
def test_binary(args):
    ...
    output_filename = f"results_{input_filename}.csv"
    save_path = os.path.join(args.output_dir, output_filename)
    
    print(f"--- Starting Binary Model Testing (NO Mass Diff) ---")
    ...
    # The old loader returns 4 items, grab meta dim from item [2]
    ...
    with torch.no_grad():
        # UPDATED: Unpack ONLY 4 items (No mass_diffs)
        for batch_A, batch_B, batch_meta, labels in tqdm(test_loader, desc="Testing"):
            ...
            # UPDATED: No mass_diffs in model call
    ...
    try:
        ...
        roc = 0.5 # Handle single-class edge cases
