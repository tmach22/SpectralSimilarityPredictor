...
# --- IMPORTS UPDATED FOR NO MASS DIFF ---
# Ensure this matches your file name for the reverted model
from classifier_siamesemodel import SiameseSpectralSimilarityModel 
# Ensure this matches your file name for the reverted loader
from binary_data_loader_wo_mass_diff import BinaryClassificationDataset, binary_collate_fn
...
def setup_differential_fine_tuning(model, verbose=True):
    ...
    - Unfreezes Similarity Head ONLY (No Mass Gate).
    """
    ...
    # 3. Ensure Head is ALWAYS trainable
    if verbose: print(" -> Unfreezing Similarity Head")
    for param in model.similarity_head.parameters(): param.requires_grad = True
    
    # REMOVED: Mass Gate Unfreezing (Doesn't exist in old model)
...
def train_binary(args):
    ...
    print(f"--- Starting Binary Training (Focal Loss + Diff. Fine-Tuning + NO Mass Diff) ---")
    ...
    # The old loader returns 4 items, we need the 3rd one for meta dim
    ...
    if args.freeze_encoder:
        ...
        # UPDATED: Only head parameters (No mass gate)
        # Note: model.similarity_head matches the name in your provided model script
        head_params = list(model.similarity_head.parameters())
    ...
    best_val_auc = 0.0
    save_name = "best_focal_model_nomassdiff.pth"
    ...
    for epoch in range(args.epochs):
        ...
        # UPDATED LOOP: Unpack only 4 items
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_dir", type=str, default="./focal_model_nomass")
