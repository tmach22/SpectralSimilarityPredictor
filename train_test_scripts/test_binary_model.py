...
def test_binary(args):
    ...
    print(f"--- Starting Binary Model Testing (With Mass Gating) ---")
    ...
    with torch.no_grad():
        # UPDATED: Unpack 5 items from loader
        for batch_A, batch_B, batch_meta, mass_diffs, labels in tqdm(test_loader, desc="Testing"):
            ...
            mass_diffs = mass_diffs.to(device) # <--- Move Mass Diff to GPU
            
            # UPDATED: Pass mass_diffs to model
