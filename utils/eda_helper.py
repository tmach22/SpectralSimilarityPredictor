    ...
    if is_inter:
        ...
        try:
            ...
            if not clean_df.empty:
                pearson_corr = clean_df['tanimoto'].corr(clean_df['cosine_similarity'], method='pearson')
                spearman_corr = clean_df['tanimoto'].corr(clean_df['cosine_similarity'], method='spearman')
                report += f"  Tanimoto vs. Cosine Correlation (Pearson / Spearman): {pearson_corr:.4f} / {spearman_corr:.4f} (on {len(clean_df):,} clean rows)\n"
            else:
                report += "  Tanimoto vs. Cosine Correlation: N/A (data was all NaN)\n"
...
def plot_distributions(df, taxon_name, plot_dir, is_inter=False):
    ...
    safe_taxon_name = taxon_name.replace(":", "").replace("→", "to").replace(" ", "_").replace("/", "_")
    ...
    try:
        ...
        # Catch plotting errors, e.g., "No such keys(s): 'mode.use_inf_as_null'"
