...
def siamese_collate_fn(batch):
    ...
    targets_sim = torch.stack([item[2] for item in batch])
    targets_label = torch.stack([item[3] for item in batch])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Test the Multi-Task Siamese Dataloader")
    ...
    print("\n--- Initiating Multi-Task Dataloader Test ---")
    ...
    try:
        ...
        batch_A, batch_B, targets_sim, targets_label = next(batch_iterator)
        ...
        print("\n--- Batch Structure ---")
        print(f"Targets (Continuous Entropy Sim) : {targets_sim.shape} -> Example: {targets_sim[0]:.4f}")
        print(f"Targets (Binary Entropy Label)   : {targets_label.shape} -> Example: {targets_label[0]:.0f}")
        ...
        print("\n[+] Dataloader test passed successfully! You are ready for Phase 3 Multi-Task Training.")
